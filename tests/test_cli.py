...
class TestCli(unittest.TestCase):
    ...
    def test_configure_command_commit_change(self):
        ...
        # Modify yagso.yaml to change lib3/bis commit
        path = Path('tests/sample1/yagso_test_root/yagso.yaml')
        manager = ManifestManager()
        manifest = manager.load_manifest(path)
        new_manifest = copy.deepcopy(manifest)
        manager.update_submodule_field(new_manifest, 'lib3/bis', 'commit', 'develop/YAGSO')

        # Write modified manifest back
        manager.save_manifest(new_manifest, Path('tests/sample1/yagso_test_root/yagso.yaml'))
        ...
        try:
            ...
            manager.save_manifest(manifest, Path('tests/sample1/yagso_test_root/yagso.yaml'))
