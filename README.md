...
Instead of manually tracking which branch each submodule should be on, 
you declare it once in the YAML manifest generated by the `generate` command.
You can change on the manifest any submodules properties, add/remove submodule and then ask yagso to
apply the manifest with `configure` command.

Any submodule modification is bumped to the hierarchy with `commit` command.

`push` and `update` are git wrapper that proceed recursively on submodules.

Additionally a Bill Of Material can be generated with `generate` command with --BOM option.
